...
**Goal:** Build a `/invoices/:id` detail page with view/edit modes and status workflow actions (Approve, Lock, Cancel).

**Architecture:** A single new React component `InvoiceDetail.tsx` handles both view and edit modes. View mode shows all invoice fields read-only. Edit mode (draft only) reuses the same line-item table logic from `InvoiceForm`. `App.tsx` is updated to point `/invoices/:id` to the new component instead of `InvoiceList`.

**Tech Stack:** React 18, React Router v7, Ant Design 5, TypeScript, existing `api.ts` service

---

### Task 1: Create InvoiceDetail component

**Files:**
- Create: `client/src/pages/invoices/InvoiceDetail.tsx`
- Modify: `client/src/App.tsx:56`

- [ ] **Step 1: Create `client/src/pages/invoices/InvoiceDetail.tsx`**
    ...
    <div>
      ...
      <Card title="Line Items" style={{ marginBottom: 16 }}>
        <Table
          ...
          dataSource={editMode ? editItems : invoice.items}
...
- [ ] **Step 2: Update `client/src/App.tsx` — import and wire the new route**

Replace lines 16 and 56 in `App.tsx`:

```tsx
// Add import after InvoiceForm import (line 16):
...
```bash
cd /var/www/vat/client && npm run build
```

Expected: `dist/` folder updated with no TypeScript errors.

- [ ] **Step 4: Commit**

```bash
cd /var/www/vat
git add client/src/pages/invoices/InvoiceDetail.tsx client/src/App.tsx
git commit -m "feat: add invoice detail page with view/edit/approve/lock/cancel"
```
