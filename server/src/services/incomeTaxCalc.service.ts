/**
 * Individual Income Tax Calculation Engine — pure functions, no DB access.
 * Bangladesh NBR slabs for FY 2025-26 (Assessment Year 2026-27).
 *
 * IMPORTANT: This engine is duplicated in client/src/utils/incomeTaxCalc.ts and
 * must be kept in sync manually (same convention as the VAT calc engine).
 */
...
/** Tax-free threshold (the first 0% band) by taxpayer category, in Taka. */
...
/** Minimum tax floor by taxpayer status, in Taka. */
export const MINIMUM_TAX: Record<IncomeTaxpayerStatus, number> = {
  existing: 5000,
  new: 1000,
};

/** Progressive band widths above the tax-free threshold (fixed for every category). */
...
export interface SlabBreakdownRow {
  label: string;       // e.g. "Tax-free", "Next 3,00,000 @ 10%"
  slabAmount: number;  // income taxed in this band
  rate: number;        // percentage, e.g. 10
  tax: number;         // tax from this band
}
...
/** Compute gross income tax (before minimum-tax floor and advance-tax credit). */
...
/** Full computation: slabs → minimum-tax floor → advance-tax/TDS credit. */
