...
import { getArSummary } from '../../services/payment';
...
export default function ArPage() {
  ...
  useEffect(() => {
    getArSummary()
      .then(setData)
      .catch(() => message.error('Failed to load AR summary'))
  ...
  const totals = data.reduce(
    ...
    { totalInvoiced: 0, totalPaid: 0, outstanding: 0, current: 0, days31_60: 0, days61_90: 0, over90: 0 }
  );
  ...
  const columns = [
    {
      ...
      render: (_: unknown, r: AgingEntry) => (
        <span style={{ fontFamily: D.manrope, fontWeight: 600, color: D.onSurface }}>
          {r.customerName || <span style={{ color: D.onSurfaceVar, fontStyle: 'italic' }}>No customer</span>}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      align: 'right' as const,
      render: (_: unknown, r: AgingEntry) => (
        <span style={{ fontFamily: D.manrope, fontWeight: 700, color: r.outstanding > 0 ? D.red : D.tertiary }}>
          {fmt(r.outstanding)}
        </span>
      ),
    },
  ...
  return (
    <div>
      <PageHeader
        ...
        sub="Outstanding balances from sales invoices, grouped by customer"
      />

      {/* Summary strip */}
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 24 }}>
        {[
          { label: 'Total Invoiced', value: fmt(totals.totalInvoiced), color: D.onSurface },
          { label: 'Total Paid', value: fmt(totals.totalPaid), color: D.tertiary },
          { label: 'Outstanding', value: fmt(totals.outstanding), color: D.red },
