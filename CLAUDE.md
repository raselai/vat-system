...
Monorepo with two packages — `client/` (React SPA) and `server/` (Express API). No shared code package; two calculation engines are duplicated across client and server and must be kept in sync manually: the VAT engine (`server/src/services/vatCalc.service.ts` ↔ `client/src/utils/vatCalc.ts`) and the income-tax engine (`server/src/services/incomeTaxCalc.service.ts` ↔ `client/src/utils/incomeTaxCalc.ts`).
...
### Income Tax Module (`/api/v1/income-tax`)

Personal **individual income-tax calculator** — saves one computation per user per assessment year. Distinct from TDS (which is monthly deduction-at-source) and from the VAT modules: this covers the annual income-tax slab calculation for a person, not a company.

- **User-scoped, NOT company-scoped**: routes mount `authenticate` only — no `companyScope`, no `rbac`, no `auditLog` (mirrors the `PUT /auth/me` pattern). Every query is filtered by `req.user.userId`. There is no `x-company-id` involvement
- **Route-mounting order matters**: `/api/v1/income-tax` must be registered **before** the import/export routes in `app.ts`, otherwise the company-scope guard on those routes intercepts and 403s it
- **Calc engine** (`server/src/services/incomeTaxCalc.service.ts`): pure functions, no DB. Bangladesh NBR FY 2025-26 slabs (Assessment Year 2026-27). **Duplicated in `client/src/utils/incomeTaxCalc.ts`** — keep in sync manually, same convention as the VAT engine
  - Tax-free threshold varies by `category`: `general` 375k, `women_senior` 425k, `third_gender_disabled` 500k, `freedom_fighter` 525k
  - Progressive bands above the threshold are fixed for all categories: 10% / 15% / 20% / 25% / 30%
  - **Minimum-tax floor** by `taxpayerStatus`: `existing` ৳5,000, `new` ৳1,000 — applied only when `subjectToMin` and `taxableIncome > threshold`. Final tax = `max(grossTax, applicableMinimum)`, then subtract `advanceTaxPaid` → `netPayable` / `refundable`
